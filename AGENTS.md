...
- **游戏（game）出门（2026-09-24 定）**：按「出去一次」计，不按每把计——出门基础 20 分（tasks.json `game-outing`）；当天 表现分 净涨每 1 点加 5 分（写一笔 earn，note 注明涨前涨后值）。用户当前处于 表现分 平台期，此系数不设上限；若未来脱瓶涨速大增再议调整。日常顺手一把、不出门不另计。
