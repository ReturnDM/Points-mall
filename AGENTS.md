...
> 本文件是项目技术文档（随 git 公开）；**个人计分细则、商城定位等私人内容一律不写这里**，
> 统一放数据目录 `积分规则.md`（随坚果云私有同步）。
>
> 管家侧的 points-ledger skill 为薄指针版（2026-09-24 起，见管家库 `.dsh/skills/points-ledger/`）：
> skill 只做路由，规则正文始终以本项目实时文件为准，本项目更新后管家侧自动生效，无需同步。
